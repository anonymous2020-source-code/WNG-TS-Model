...
class MLPnet(torch.nn.Module):
      def __init__(self, node_number, batch_size, k_hop):
          ...
          self.k_hop = k_hop
          self.aggregate_weightT = torch.ones(1, 1, node_number)
          self.aggregate_weightF = torch.ones(1, 1, node_number)
      ...
      def forward(self, x_T, x_F):
          ...
          x_F = torch.matmul(self.aggregate_weightF, x_F)
...
def model_main(train_T, train_F, train_label, val_T, val_F, val_label, seed, method_name, dataset):
    ...
    model = MLPnet(args.node_number, args.batch_size, args.k_hop)
    ...
    for epoch in range(num_epoch):
        ...
        with open('save_time_TF/{}_{}_epoch_MLP.txt'.format(method_name, dataset), 'a+') as f:
        ...
        if (val_acc > best_acc):
            torch.save(model.state_dict(), 'save/{}_{}_MLP.pth'.format(method_name, dataset))
            best_acc = val_acc
