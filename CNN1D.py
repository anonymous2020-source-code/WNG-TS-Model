...
class CNN1Dnet(torch.nn.Module):
      ...
      def forward(self, x_T, x_F):
          ...
          x_F = torch.matmul(self.aggregate_weight, x_F)
          x_F = self.conv11(x_F)
          x_F = self.conv21(x_F)
          x_F = self.conv31(x_F)
          x_F = self.conv41(x_F)
...
def model_main(train_T, train_F, train_label, val_T, val_F, val_label, seed, method_name, dataset):
    ...
    #train_T = torch.unsqueeze(train_T, dim=1).float()
    train_F = torch.FloatTensor(train_F)
    #train_F = torch.unsqueeze(train_F, dim=1).float()
    val_T = torch.FloatTensor(val_T)
    #val_T = torch.unsqueeze(val_T, dim=1).float()
    val_F = torch.FloatTensor(val_F)
    #val_F = torch.unsqueeze(val_F, dim=1).float()
    ...
    model = CNN1Dnet(args.node_number, args.batch_size, args.k_hop)
    ...
    for epoch in range(num_epoch):
        ...
        with open('save_time_TF/{}_{}_epoch_CNN1D.txt'.format(method_name, dataset), 'a+') as f:
        ...
        if (val_acc > best_acc):
            torch.save(model.state_dict(), 'save/{}_{}_CNN1D.pth'.format(method_name, dataset))
            best_acc = val_acc
